...
/* ══════════════════════════════════════════
   MAIN APP  — met permanente opslag
══════════════════════════════════════════ */
export default function App() {
  const [session, setSession]    = useState(null);
  const [klanten, setKlantenRaw] = useState(INIT_KLANTEN);
  const [soepen,  setSoepenRaw]  = useState(INIT_SOEPEN);
  ...
  useEffect(() => {
    async function load() {
      try {
        const [rK, rS, rW] = await Promise.all([
          window.storage.get("soepkelder:klanten",  true).catch(() => null),
          window.storage.get("soepkelder:soepen",   true).catch(() => null),
          window.storage.get("soepkelder:weekmenu", true).catch(() => null),
        ]);
        if (rK) setKlantenRaw(JSON.parse(rK.value));
        if (rS) setSoepenRaw(JSON.parse(rS.value));
        if (rW) setWeekMenuRaw(JSON.parse(rW.value));
      } catch (_) { /* eerste keer: geen data */ }
      setLoaded(true);
    }
    load();
  }, []);

  /* ── wrapper-setters die ook opslaan ── */
  async function setKlanten(val) {
    setKlantenRaw(val);
    try { await window.storage.set("soepkelder:klanten", JSON.stringify(val), true); } catch(_) {}
  }
  async function setSoepen(val) {
    setSoepenRaw(val);
    try { await window.storage.set("soepkelder:soepen", JSON.stringify(val), true); } catch(_) {}
  }
  async function setWeekMenu(val) {
    setWeekMenuRaw(val);
    try { await window.storage.set("soepkelder:weekmenu", JSON.stringify(val), true); } catch(_) {}
  }

  /* ── laadscherm ── */
  if (!loaded) return (
    <div style={{ fontFamily: "Lora, serif", minHeight: "100vh", background: C.cream, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      <style>{BASE_STYLE}</style>
